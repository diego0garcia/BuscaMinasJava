...
public class SelectCharacterController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        button_remove.disableProperty()
                .bind(Bindings.equal(-1, tableView_main.getSelectionModel().selectedIndexProperty()));

        button_next.disableProperty().bind(Bindings.or(
                Bindings.equal(-1, tableView_main.getSelectionModel().selectedIndexProperty()),
                Bindings.or(Bindings.equal(-1, dificulty), Bindings.not(
                        Bindings.equal(1, Bindings.size(tableView_main.getSelectionModel().getSelectedItems()))))));

        button_modify.disableProperty().bind(Bindings.or(
                Bindings.equal(-1, tableView_main.getSelectionModel().selectedIndexProperty()),
                Bindings.not(
                        Bindings.equal(1, Bindings.size(tableView_main.getSelectionModel().getSelectedItems())))));
        ...
        dificulty.addListener(evnt -> {
            ...
            text_informationAboutDificulty.setText("Dificultad: "
                    + ((getDificulty() == 1) ? "Principiante"
                            : (getDificulty() == 2) ? "Intermedio"
                                    : (getDificulty() == 3) ? "Experto" : "Personalizado")
                    + " | Tablero: " + getFilas() + "x" + getColumnas() + " | Minas: " + getMinas());
        });
    }
    ...
    private void modifyAction() {
        try {
            ...
            Parent root = miCargador.load();
            EditPlayerController controladorPersona = miCargador.<EditPlayerController>getController();
            root.getStylesheets().add(getClass().getResource("/styles/style.css").toExternalForm());
    ...
    private void addDificulty() {
        try {
            ...
            FXMLLoader miCargador = new FXMLLoader(
                    getClass().getResource("/es/diego/buscaminas/addCustomDificulty.fxml"));
            Parent root = miCargador.load();
